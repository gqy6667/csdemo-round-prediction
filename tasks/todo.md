# ROUNDCAST v1 — 可执行任务清单（T01–T05 已验收）
...
当前已实现并验收 T01–T05；后端 12 项真实组合均已通过，网页仍保持 T04 的案例 A／购买结束／XGBoost 预览。下一项 T06 开放页面选择与对比，两个分析视图尚未制作。代码、测试和验收记录仅本地保存，未提交或推送。详见 [T05 验收记录](../reports/roundcast_interactive_v1/t05_verification.md) 与 [启动说明](../reports/roundcast_interactive_v1/README.md)。
...
证据：[对话验收](../reports/roundcast_interactive_v1/chat_verification.md)。这不是 T06；后续主线顺序不变。
...
- 当前 T01–T05 的服务、四模型推理、启动模块、最小观众网页及测试已实现；T06–T11 仍是后续目标，不代表已经通过。
- 本任务清单确认后进入实现；不再重开已批准的产品范围或 P0–P4 顺序。发现需要新增依赖、改变模型或扩大范围时再提出。
- 之前的 GitHub 授权对应 eb0a0e3 四份规划文档，不视作后续自动推送授权。录制视频、公开部署和换机备份也不在本清单内。
...
- [ ] 完成 T10。
  ...
  - 验收：12 项参考比对、六个 API 类别、请求拒绝、服务停止、缺工件、坏哈希、快速切换、重复点击、部分失败、三页状态/揭示边界全部通过；浏览器无控制台错误，冻结工件/数据哈希不变。
...
用户已批准并完成 T01–T05；T04 首个可预览真实链路已交付，T05 完整后端矩阵已通过，下一项为 T06。T06–T11 未开始，不因后端矩阵通过就勾选网页切换或三页完成状态。
