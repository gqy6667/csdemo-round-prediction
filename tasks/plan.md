...
日期：2026-09-05。状态：规格及实施顺序均已获用户批准，本轮文档同步 GitHub 已获授权；尚未写入网页或模型服务代码。
...
P0 的案例资格只读预检已完成，模型运行时加载/推理仍待实现阶段验证。
...
本计划已获批准；下一阶段把具体任务写入 todo，单项尽量不超过约五个文件。
...
- 下一步：细化可执行任务及各项验收，再进入实施。
- 本轮授权：提交并推送规格、实施计划、案例预检和进度四份文档。
- 本轮不训练、不启动服务、不修改网页、不录视频；文档同步不代表交互网页已完成。
- 本轮文档提交/推送已获授权；后续录制/发布视频、公开部署、新依赖或扩大数据范围仍保留规格中的先询问边界。
