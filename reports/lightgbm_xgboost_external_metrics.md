...
| 开局前 | LightGBM 固定评估 M24 | 0.650767 | 0.727846 | 0.591437 | 0.205201 | 0.018875 |
...
检验。M24 已补做 2,000 次系列赛级配对 bootstrap，结果如下：

| 指标 | LightGBM 性能优势 | 配对 95% CI | 区间排除 0 |
|---|---:|---:|---|
| Accuracy | +0.003356 | [-0.001508, +0.008273] | 否 |
| AUC | +0.000724 | [-0.001289, +0.002878] | 否 |
| Log Loss | +0.000296 | [-0.000909, +0.001687] | 否 |
| Brier | +0.000094 | [-0.000291, +0.000467] | 否 |
| ECE10 | +0.004323 | [-0.003503, +0.013126] | 否 |

因此准确结论更新为“LightGBM 点估计略优，但当前 79 个测试系列不足以证明稳定优势”。
...
1. 开局前 LightGBM 五项点指标略优于开局前最终 XGBoost，但五项配对 95% CI 都包含
   0，当前没有稳定胜出的统计证据；
2. M23 的 36 个候选没有可靠改善，保留 M22 是正确实验结果；
3. 开局前逻辑回归 Accuracy 最高，说明复杂树模型尚未形成明显排序优势；
4. 首杀后最终 XGBoost 是当前项目数值最强的模型，但任务本身拥有更多信息；
5. M23 之后没有继续根据 test 扩大参数搜索，而是按冻结计划进入 M24 统计评估；
6. M24 已完成上述评估：AUC 95% CI `[0.714169, 0.741427]`，Log Loss 95% CI
   `[0.580635, 0.602921]`；LAN-online 差异区间包含 0，validation OOF 选择不校准。
7. 下一步 M25 应解释 LightGBM 的 gain、Permutation、SHAP 和泄漏合同，并与
   XGBoost 的解释结果对照。
