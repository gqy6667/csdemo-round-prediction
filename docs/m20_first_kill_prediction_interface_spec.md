...
- 10 个阻断检查全部通过，`ready_for_m21=True`；
- JSON/CSV 示例输入都通过并输出完全相同的概率；
- 示例 CT/T 概率为 `0.7187644839 / 0.2812355161`，概率和为 `1.0`；
- 27 个购买基础字段、4 个首杀字段、9 个派生差值、40 个原始特征和 82 个编码列通过；
- 8 张地图、36 个首杀武器类别和 409 棵部署树与模型 bundle 一致；
- 模型与 M19 SHA-256 一致，校准器与 M18 SHA-256 一致且绑定同一模型和数据；
- 10 个固定错误案例全部拒绝，字符串事件类型也由专项测试覆盖；
- XGBoost fit 调用为 `0`，M18 五项指标完全不变；
- M19 十项正式目标继续通过，remaining 全部为 `0`；
- 七行外部比较重新生成；131 项自动化测试和源码编译通过；
- 完整结果见 `reports/esta_full_m20/m20_first_kill_interface_report.md`。
