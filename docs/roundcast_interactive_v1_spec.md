...
状态：产品范围、技术规格与实施顺序均已获用户确认，首轮规划文档已按授权同步至 GitHub（eb0a0e3）。现已完成 T01–T05：可信来源准备、本地接口、四模型共 12 项真实推理和最小观众网页。网页选择仍为 T04 的单案例；T06–T11 尚未实施。本轮实现仅本地保存，未提交或推送；旧静态页面、冻结模型、校准器与数据均未改动。[T05 验收](../reports/roundcast_interactive_v1/t05_verification.md)。
...
在项目根目录的 PowerShell 终端运行。Python 绝对路径仅为当前开发机示例，换机后需替换为对应环境路径。当前已提供准备/单例推理服务、本地启动模块和集中测试；网页仅支持案例 A／购买结束／XGBoost：
...
- [x] 点击运行确实调用模型，不能以参考概率表代替（网页为 T04 单组合；后端 T05 的 12 项均真实调用）。
...
2026-09-05：用户已批准本规格及 [实施计划](../tasks/plan.md) 的 P0–P4 顺序，首轮四份规划文档已同步为 eb0a0e3。T01–T05 已验收，提供单案例本地观众预览与完整后端 12 项推理；下一项为 T06。全套 316 项测试通过，12 项参考误差均为 0；T04 浏览器与服务断开验收保留为此前记录。页面切换、三页联动及正式指标 API 尚未完成，不宣告 v1 全部完成；没有新增 GitHub 提交/推送、重训、公开部署或视频。
