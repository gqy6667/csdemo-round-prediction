...
状态（2026-09-06）：产品范围、技术规格与实施顺序均已获用户确认，T01–T11 已完成。用户继续“下一步 网页”后，三页最终截图、六页 PDF、三例演示及启动/关闭说明已交付；12 项实际浏览器预测与参考误差为 0，重启复验通过。T10 的 357 项 Python / 39 项 JavaScript 回归记录保留，本轮未改应用逻辑或冻结来源。此前已同步至 T06（db6242a）；本次同步提交按用户“上传至 github”的要求补齐 T07–T11。[T11 交付验收](../reports/roundcast_interactive_v1/t11_verification.md)。
...
T09 已在同一个入口开放观众/比赛分析/技术分析三个视图。URL 片段仅保存 `view / case / stage / algorithm` 的固定白名单值；刷新只恢复选择，不恢复预测、赛果或聊天，也不自动运行。前进/后退与视图切换共享同一控制器；同案例已有结果可复用，跨案例仍清空。正式指标使用独立加载状态和请求代次，不阻塞或清除当前模型推理。
...
| GET /api/examples/{id} | 购买结束只读快照的兼容入口；不混入真实赢家 |
| GET /api/examples/{id}/snapshots/{stage} | 按购买结束/首杀后返回对应的只读快照；不含赛果或参考概率 |
...
在项目根目录的 PowerShell 终端运行。Python 绝对路径仅为当前开发机示例，换机后需替换为对应环境路径。当前已提供三案例/四模型推理服务、本地启动模块和集中测试；网页开放全部 12 项选择组合及当前案例的四项对比：
...
- [x] 一条本机命令可启动网页；关闭服务后再次运行或读取赛果会显示连接不可用（T04；不做后台持续探测）。
- [x] 三个视图保持案例/时点/算法一致，选择与结果组合明确（T09）。
- [x] 三个不同真实回合通过唯一键对齐两个时点且均属于冻结测试集（T01）。
- [x] 全部 12 项真实推理通过参考概率比对（T05，实际 HTTP 验收）。
- [x] 点击运行确实调用模型，不能以参考概率表代替（T06 网页 12 项组合均已实际点击并核对参考）。
- [x] 技术指标可追溯到各自正式评估集，不使用三个案例估计整体表现（T08–T09）。
- [x] 未实现数据项明确缺失；时点、阵营、输入截止及事后结果不混用（T10）。
- [x] 新测试、全套回归、编译及实际浏览器检查通过并记录（T10）。
- [x] 提供三页截图、启动/关闭说明及三个可复现演示步骤（T11，另附六页图文 PDF）。

## 12. Review Gate — 当前审阅点

2026-09-06：用户继续“下一步 网页”后，T11 已完成；T01–T11 和附加 Codex 对话的历史验收均有记录，本地交互演示 v1 已交付。最新三页图文与三例说明见 [演示指南](../reports/roundcast_interactive_v1/demo_guide.md)。本轮实际浏览器 12 项真实预测、共享状态、赛果揭示及重启推理通过；图文 PDF 六页均经渲染检查。可进入试讲/审阅与录屏准备。本轮未提交/推送、重训、公开部署或录制视频。
